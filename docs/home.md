| 단계 | 세부 단계 | 설명 |
|------|----------|-----|
| 1. 시스템 설정 | 1.1 Windows11에 Docker 설치 | Windows 11에서 Docker를 설치하고 설정하는 과정을 진행합니다. Docker는 가상 컨테이너를 이용하여 어플리케이션을 실행하는 플랫폼으로, 다양한 환경에서 동일한 결과를 얻을 수 있도록 도와줍니다. |
...
|  | 1.4 ROS2 (Galactic 이상 버전) Docker 이미지 다운로드 및 commit | ROS2 (Galactic 이상 버전)의 Docker 이미지를 다운로드하고, 이를 리포지토리에 commit합니다. |
